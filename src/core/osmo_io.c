...
{
	char *backend = getenv("LIBOSMO_IO_BACKEND");
	if (backend == NULL)
	...
	if (!strcmp("POLL", backend)) {
		...
		fprintf(stderr, "Invalid LIBOSMO_IO_BACKEND selected (got: %s)\n", backend);
		exit(1);
	}
...
int osmo_iofd_write_msgb(struct osmo_io_fd *iofd, struct msgb *msg)
{
	...
	if (rc < 0) {
		...
		LOGP(DLIO, LOGL_ERROR, "iofd(%s) enqueueing message failed (%d). Rejecting msgb\n", iofd->name, rc);
		return rc;
	}
		  ...
		  const struct osmo_io_ops *ioops, void *data)
{
	...
	LOGP(DLIO, LOGL_DEBUG, "iofd(%s) fd=%d using backend %s\n", iofd->name, iofd->fd, osmo_io_backend_name(g_io_backend));
...
/*! Close the iofd
 *
 *  This function closes the underlyiny fd and clears any messages in the tx queue
